...
# Export mode: 'mysql' or 'excel'
EXPORT_MODE = 'mysql'  # Change to 'excel' if needed
...
CHART_FORMULAS = {
    "RFT - CNC": "RFT_CNC = Nb_RFT / Nb_Ordres",
    "RFT - Assembly": "RFT_Assembly = Nb_Conforme / Nb_Total",
    "Global RFT": "RFT_Global = RFT_CNC * RFT_Assembly",
    "CNC Error Rate": "Taux_Erreur_CNC_% = Nombre_Defauts / Pieces_Controlees * 100",
    "Pareto - Defect Types": "Nombre_Defauts grouped by Description_Defaut",
    "Pareto - Corrective Actions": "Nombre_Defauts grouped by Action_Correction",
    "Defects by Origin": "Nombre_Defauts grouped by Origine",
    "Scrap/Rework Tracking": "Total = SUM(total) grouped by Type_Action and period",
    "Non-Conformities by Origin": "Nb_Non_Conformites grouped by Origine",
    "Production Trend": "Quantite_Produite over time",
    "Production by Client": "Quantite_Produite grouped by Client",
}
...
def clear_mysql_tables(conn):
    """Clear all KPI tables before inserting new data."""
    cursor = conn.cursor()
    tables = [
        'kpi_suivi_journalier',
        'kpi_cnc_mensuel', 'kpi_cnc_rft_hebdomadaire', 'kpi_cnc_pareto_defauts',
        'kpi_cnc_pareto_actions', 'kpi_cnc_par_operateur', 'kpi_cnc_par_statut',
        'kpi_cnc_par_piece', 'kpi_cnc_par_origine',
        'kpi_cf_mensuel', 'kpi_cf_scrap_rework', 'kpi_cf_pareto_actions',
        'kpi_cf_par_origine', 'kpi_cf_par_technicien',
        'kpi_mmc_journalier', 'kpi_mmc_finishgood_mensuel', 'kpi_mmc_par_client',
        'kpi_mmc_par_operateur', 'kpi_mmc_tests',
        'kpi_kpi_mensuel', 'kpi_rft_global', 'kpi_kpi_global',
    ]
    for table in tables:
        try:
...
def insert_dataframe_to_table(conn, df, table_name, column_mapping=None):
    """Insert DataFrame into MySQL table. Creates table if needed."""
    ...
    try:
        ...
        df_clean = df.copy()
        df_clean.columns = [str(col).replace('%', 'pct').replace('/', '_').replace(' ', '_').lower() 
        ...
        if column_mapping:
            mapping_clean = {k.replace('%', 'pct').replace('/', '_').replace(' ', '_').lower(): v 
                           for k, v in column_mapping.items()}
            df_clean = df_clean.rename(columns=mapping_clean)
            db_cols = list(mapping_clean.values())
        else:
        ...
        create_sql = f"CREATE TABLE `{table_name}` ({', '.join(col_defs)}, PRIMARY KEY (id)) AUTO_INCREMENT = 1" \
                    if 'id' in db_cols else f"CREATE TABLE `{table_name}` ({', '.join(col_defs)})"
        ...
        for _, row in df_clean.iterrows():
            values = [row.get(col, None) if col in df_clean.columns else None for col in db_cols]
...
def export_to_mysql(suivi, cnc_results, fc_results, mmc_results, global_results):
    """Export all KPI data to MySQL database."""
    print(f"\n--- Exporting to MySQL ---")
    
    conn = get_mysql_connection()
    if not conn:
        print("[ERROR] Failed to connect to MySQL. Aborting export.")
        return False
    
    try:
        # Clear existing data
        print("[*] Clearing existing data from tables...")
        clear_mysql_tables(conn)
        
        # Export Suivi Journalier
        cols_map = {
            'KW': 'kw', 'Mois': 'mois', 'Date': 'date', 'Detecter par': 'detecter_par',
            'Controler par': 'controler_par', 'N°Order': 'numero_order', 'Statu': 'statu',
            'Nombre pieces controlees': 'nombre_pieces_controlees', 'Nombre defauts': 'nombre_defauts',
            'Piece NIO': 'piece_nio', 'Type technique du defaut': 'type_technique_defaut',
            'Description du defaut': 'description_defaut', 'N°QRQC': 'numero_qrqc',
            'Action de correction': 'action_correction', 'Origine de defaut': 'origine_defaut', 'Etat': 'etat'
        }
        rows = insert_dataframe_to_table(conn, suivi, 'kpi_suivi_journalier', cols_map)
        print(f"    [+] kpi_suivi_journalier: {rows} rows")
        
        # Export CNC KPIs
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Mensuel'), 'kpi_cnc_mensuel')
        print(f"    [+] kpi_cnc_mensuel: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_RFT_Hebdomadaire'), 'kpi_cnc_rft_hebdomadaire')
        print(f"    [+] kpi_cnc_rft_hebdomadaire: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Pareto_Defauts'), 'kpi_cnc_pareto_defauts')
        print(f"    [+] kpi_cnc_pareto_defauts: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Pareto_Actions'), 'kpi_cnc_pareto_actions')
        print(f"    [+] kpi_cnc_pareto_actions: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Par_Operateur'), 'kpi_cnc_par_operateur')
        print(f"    [+] kpi_cnc_par_operateur: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Par_Statut'), 'kpi_cnc_par_statut')
        print(f"    [+] kpi_cnc_par_statut: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Par_Piece'), 'kpi_cnc_par_piece')
        print(f"    [+] kpi_cnc_par_piece: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, cnc_results.get('CNC_Par_Origine'), 'kpi_cnc_par_origine')
        print(f"    [+] kpi_cnc_par_origine: {rows} rows")
        
        # Export Final Control KPIs
        rows = insert_dataframe_to_table(conn, fc_results.get('CF_Mensuel'), 'kpi_cf_mensuel')
        print(f"    [+] kpi_cf_mensuel: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, fc_results.get('CF_Scrap_Rework'), 'kpi_cf_scrap_rework')
        print(f"    [+] kpi_cf_scrap_rework: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, fc_results.get('CF_Pareto_Actions'), 'kpi_cf_pareto_actions')
        print(f"    [+] kpi_cf_pareto_actions: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, fc_results.get('CF_Par_Origine'), 'kpi_cf_par_origine')
        print(f"    [+] kpi_cf_par_origine: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, fc_results.get('CF_Par_Technicien'), 'kpi_cf_par_technicien')
        print(f"    [+] kpi_cf_par_technicien: {rows} rows")
        
        # Export MMC KPIs
        rows = insert_dataframe_to_table(conn, mmc_results.get('MMC_Journalier'), 'kpi_mmc_journalier')
        print(f"    [+] kpi_mmc_journalier: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, mmc_results.get('MMC_FinishGood_Mensuel'), 'kpi_mmc_finishgood_mensuel')
        print(f"    [+] kpi_mmc_finishgood_mensuel: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, mmc_results.get('MMC_Par_Client'), 'kpi_mmc_par_client')
        print(f"    [+] kpi_mmc_par_client: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, mmc_results.get('MMC_Par_Operateur'), 'kpi_mmc_par_operateur')
        print(f"    [+] kpi_mmc_par_operateur: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, mmc_results.get('MMC_Tests'), 'kpi_mmc_tests')
        print(f"    [+] kpi_mmc_tests: {rows} rows")
        
        # Export Global KPIs
        rows = insert_dataframe_to_table(conn, global_results.get('KPI_Mensuel'), 'kpi_kpi_mensuel')
        print(f"    [+] kpi_kpi_mensuel: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, global_results.get('RFT_Global'), 'kpi_rft_global')
        print(f"    [+] kpi_rft_global: {rows} rows")
        
        rows = insert_dataframe_to_table(conn, global_results.get('KPI_Global'), 'kpi_kpi_global')
        print(f"    [+] kpi_kpi_global: {rows} rows")
        
        print(f"\n[SUCCESS] All KPI data exported to MySQL database 'kpi_dashboard'")
        return True
    except Exception as e:
        print(f"[ERROR] Export to MySQL failed: {e}")
        return False
    finally:
        conn.close()
...
def export_to_excel(suivi, cnc_results, fc_results, mmc_results, global_results):
    """Export to Excel (original function - kept for backward compatibility)."""
...
def main():
    ...
    if EXPORT_MODE == 'mysql':
        export_to_mysql(suivi, cnc_results, fc_results, mmc_results, global_results)
    else:
        export_to_excel(suivi, cnc_results, fc_results, mmc_results, global_results)

    print("\n" + "=" * 60)
    if EXPORT_MODE == 'mysql':
        print("  DONE. MySQL database updated with latest KPI data.")
        print("  Database: kpi_dashboard")
    else:
